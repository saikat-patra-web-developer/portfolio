  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-3 sm:px-6 pointer-events-none">
      <nav
        ...
        className={`pointer-events-auto w-full max-w-[1120px] transition-all duration-300 mt-2 sm:mt-3.5 rounded-2xl border flex items-center justify-between px-3.5 sm:px-6 ${scrolled
            ? "py-2 sm:py-3 bg-[#020B18]/90 backdrop-blur-2xl border-[#00E5FF]/30 shadow-[0_10px_35px_rgba(0,0,0,0.8),0_0_20px_rgba(0,229,255,0.1)]"
            : "py-2.5 sm:py-4 backdrop-blur-xl border-[#00E5FF]/20 shadow-[0_8px_30px_rgba(0,0,0,0.5),0_0_15px_rgba(0,229,255,0.05)]"
          }`}
      >
        {/* Brand */}
        <Link
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.name}
                to={link.path}
                className={`relative px-4 py-1.5 text-xs xl:text-sm font-semibold rounded-full transition-all duration-200 select-none ${active
